...
func _bind_ui() -> void:
	...
	score_label = get_node_or_null("CanvasLayer/PlayUI/ScoreLabel")
	prompt_label = get_node_or_null("CanvasLayer/PlayUI/PromptLabel")
	judge_label = get_node_or_null("CanvasLayer/PlayUI/JudgeLabel")
	result_title_label = get_node_or_null("CanvasLayer/ResultUI/Panel/ResultTitleLabel")
	result_score_label = get_node_or_null("CanvasLayer/ResultUI/Panel/ResultScoreLabel")
	result_breakdown_label = get_node_or_null("CanvasLayer/ResultUI/Panel/ResultBreakdownLabel")
	result_rank_label = get_node_or_null("CanvasLayer/ResultUI/Panel/ResultRankLabel")
	result_guide_label = get_node_or_null("CanvasLayer/ResultUI/Panel/ResultGuideLabel")
...
func _on_score_changed(new_score: int, _perfect: int, _good: int, _miss: int) -> void:
	if score_label != null: score_label.text = "Score: %d" % new_score
...
func _on_result_ready(new_score: int, perfect: int, good: int, miss: int, rank: String, cleared: bool) -> void:
	...
	if result_title_label != null: result_title_label.text = "CLEAR" if cleared else "FAILED"
	if result_score_label != null: result_score_label.text = "Score: %d / %d" % [new_score, game_manager.max_score]
	if result_breakdown_label != null: result_breakdown_label.text = "Perfect: %d   Good: %d   Miss: %d" % [perfect, good, miss]
	if result_rank_label != null: result_rank_label.text = "Rank: %s   Clear Score: %d" % [rank, game_manager.clear_score]
...
func _set_judge_text(text: String) -> void:
	if judge_label != null: judge_label.text = text
